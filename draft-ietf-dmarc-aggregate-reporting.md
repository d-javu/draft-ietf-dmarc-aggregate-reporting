...
5. "record": **REQUIRED**
    ...
    1. "row": **REQUIRED**
        ...
        * "policy_evaluated": **REQUIRED**
            ...
            The result of applying the DMARC policy.  If alignment
...
The column marked "#" shows the number of elements allowed by the
format. An empty value means **OPTIONAL**, zero or one element;
"R" means **REQUIRED**, one element; "*" means zero or more elements,
...
Element name      | # | Content
------------------|---|--------------
version           |   | **MUST** have the value 1.0.
report_metadata   | R | Report generator metadata, see (#xml-report-metadata).
policy_published  | R | The DMARC Policy Record configuration observed by the receiving system, see (#xml-policy-published).
extension         |   | Content not specified; allows for future extensibility.
...
error             |   | Error messages encountered when processing the DMARC Policy Record, see (#error).
generator         |   | The name and version of the report generator; this can help the Report Consumer find out where to report bugs.
...
discovery_method  |   | The method used to discover the DMARC Policy Record used during evaluation.
p                 | R | A Domain Owner Assessment Policy.
sp                |   | A Domain Owner Assessment Policy.
np                |   | A Domain Owner Assessment Policy.
fo                |   | The value for the failure reporting options.
adkim             |   | The DKIM Identifier Alignment mode.
aspf              |   | The SPF Identifier Alignment mode.
testing           |   | The value of the "t" tag.
...
The results of applying DMARC. If alignment fails and the policy
applied does not match the Policy Domain's configured policy, the
"reason" element **MUST** be included.
...
envelope_from     |   | The RFC5321.MailFrom domain that the SPF check has been applied to.
envelope_to       |   | The RFC5321.RcptTo domain from the message.
...
spf               |   | SPF authentication result, see (#xml-spf).
...
human_result      |   | More descriptive information to the Domain Owner relating to evaluation failures.
...
scope             |   | The source of the domain used during validation.
...
comment           |   | Further details, if available.
