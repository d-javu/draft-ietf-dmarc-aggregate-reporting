...
The schema is described here, but see the XSD for all the details.
Within the same indentation level, numbered elements signify that
they must appear in a specific order, bulleted elements can appear
in any order.

DMARC Aggregate Reports have the XML root element "feedback" with its
default namespace set to the DMARC namespace.  For each extensions that
are used, an additional namespace with prefix **MUST** be declared in
the XML root element.

The XML root element contains up to 5 different element names,
...
5. "record": **REQUIRED**
    ...
    3. "auth_results": **REQUIRED**
        ...
        * "spf": **OPTIONAL**
            ...
            There **MAY** be zero or one "spf" elements.
